...
void echilibreaza(nod* &arb, nod* &r)
{
    ...
    if(arb)
    {
        if (arb->val == r->val)
        {
            if (dif == 2)
            {
                ...
                if (dif == 1)
                {
                    ...
                    r->st = c;
                    if (c) 
                    	r->hLeft = 1 + max(c->hLeft, c->hRight);
                    else 
                    	r->hLeft = 0;
                ...
                else
                {
                    ...
                    q->dr = r;
                    if (d) 
                    	r->hLeft = 1 + max(d->hLeft, d->hRight);
                    else 
                    	r->hLeft = 0;
                    if (c) 
                    	p->hRight = 1 + max(c->hLeft, c->hRight);
                    else 
                    	p->hRight = 0;
                ...
                if(dif == -2)
                {
                    ...
                    if (dif == -1)
                    {
                        ...
                        r->dr = b;
                        if (b) 
                        	r->hRight = 1 + max(b->hLeft, b->hRight);
                        else 
                        	r->hRight = 0;
                    ...
                    else
                    {
                        ...
                        p->st = d;
                        if (d) 
                        	p->hLeft = 1 + max(d->hLeft, d->hRight);
                        else 
                        	p->hLeft = 0;
                        if (c) 
                        	r->hRight = 1 + max(c->hLeft, c->hRight);
                        else 
                        	r->hRight = 0;
        ...
        else
        {
            ...
            p = arb->st;
            if (p) 
            	arb->hLeft = 1 + max(p->hLeft, p->hRight);
            else 
            	arb->hLeft = 0;
            p = arb->dr;
            if (p) 
            	arb->hRight = 1 + max(p->hLeft, p->hRight);
            else 
            	arb->hRight = 0;
        }
    }
